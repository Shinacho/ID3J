...
＜データ仕様とか＞
 ...
 - RIFFデータ：wavはID3ではなく、RIFFというデータ領域に情報が入っています。RIFF内の情報が、ここに入ります。RIFFの中にID3を入れることもできます。RIFF内の、ID3でないタグに書かれた日本語は文字化けします。RIFF内で、ID3は最後に書かれているものとし、RIFFタグ→ID3→RIFFタグ　という並びは想定していません。
